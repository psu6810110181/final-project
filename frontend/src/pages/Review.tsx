...
//   // --- FETCH DATA (ใช้ฟังก์ชันเดียวกับ OrderHistory) ---
//   useEffect(() => {
//     const fetchHistory = async () => {
//       try {
//         setLoading(true);
//         // ใช้ (api as any) เพื่อเลี่ยง Error กรณี Type ใน api.ts ยังไม่อัปเดต
//         const data = await (api as any).getMyOrders();
        
//         if (data && Array.isArray(data)) {
//           // กรองเอาเฉพาะออเดอร์ที่ 'completed' (สำเร็จแล้ว)
//           const completedOrders = data.filter(
//             (order: any) => order.status?.toLowerCase() === 'completed'
//           );

//           // แตกรายการ items ออกมาเป็นชิ้นๆ เหมือนที่ Shopee ทำ
//           const itemsToReview: ReviewItem[] = completedOrders.flatMap((order: any) => 
...
//       setRating(0);
//       setComment("");
//       // (Optional) อาจจะ fetch ใหม่เพื่ออัปเดตรายการที่รีวิวแล้ว
//     } catch (error) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        if (data && Array.isArray(data)) {
          // ✅ ปลดล็อคเงื่อนไข: เอาทุกออเดอร์มาแสดงเลย ไม่ต้องรอแอดมินอนุมัติ
          // (แค่ตัดออเดอร์ที่ถูก "ยกเลิก" ทิ้งไปอย่างเดียวพอ)
