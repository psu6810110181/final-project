...
import { ShoppingCart, Loader, Search, ChevronDown, FilterX, Star, Zap } from 'lucide-react'; 
...
const HomepagePromotion = () => {
      ...
      if (token) {
          api.getBookmarks().then(data => {
              if (Array.isArray(data)) {
                  setBookmarks(data.map((b: any) => b.productId || b.product?.id || b.id));
              } else if (data && Array.isArray(data.data)) {
                  setBookmarks(data.data.map((b: any) => b.productId || b.product?.id || b.id));
              }
          }).catch(() => {});
      } else {
          setBookmarks([]);
    ...
    <div className="mb-16">
        {title && (
          <div className="flex items-center gap-4 mb-8">
            <Zap className="text-orange-500 drop-shadow-md" size={32} />
            <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">{title}</h2>
          </div>
        )}
        {items.length === 0 ? (
          <div className="bg-white/50 backdrop-blur-md p-10 text-center rounded-3xl border border-white shadow-xl text-slate-500">ไม่พบสินค้าในโปรโมชันนี้</div>
        ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
                {items.map((product) => (
                <Link to={`/product/${product.id}`} key={product.id} className="group relative block">
                    <div className="bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-lg hover:shadow-2xl hover:shadow-red-500/20 transition-all duration-500 overflow-hidden border border-white/80 h-full flex flex-col relative hover:-translate-y-2">
                    
                    {product.promo && (
                        <div className="absolute top-4 left-4 bg-gradient-to-r from-red-600 to-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-full z-20 shadow-lg shadow-red-500/30 tracking-wider">
                    ...
                    <div className="h-60 overflow-hidden bg-slate-100 relative">
                        <div className="absolute inset-0 bg-gradient-to-t from-red-900/20 to-transparent z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"/>
                    ...
                    <div className="p-6 flex flex-col flex-1">
                        <div className="text-xs text-red-500 font-bold tracking-widest uppercase mb-2">{product.category || 'PROMOTION'}</div>
                        <h3 className="font-bold text-slate-800 text-xl mb-4 line-clamp-2 group-hover:text-red-600 transition-colors">{product.name}</h3>
                        ...
                        <div className="mt-auto flex items-end justify-between pt-4 border-t border-slate-100">
                            ...
                            <button onClick={(e) => handleAddToCart(e, product)} className="bg-red-50 hover:bg-red-600 hover:text-white text-red-500 p-3 rounded-2xl transition-all hover:scale-105 hover:shadow-lg hover:shadow-red-500/30">
  ...
  return (
    // ✅ เอา overflow-hidden ออกจาก Wrapper นอกสุด เพื่อให้ sticky ทำงานได้
    <div className="bg-[#F8FAFA] min-h-screen pb-20 relative">
      
      {/* ✅ ครอบ Background Orbs ด้วย overflow-hidden แยกต่างหาก */}
      ...
      <div className="sticky top-20 z-40 container mx-auto px-4 -mt-10 mb-16 transition-all duration-300">
        <div className="bg-white/95 backdrop-blur-2xl border border-white/80 shadow-2xl shadow-red-500/10 rounded-[2rem] p-4 flex flex-col lg:flex-row gap-4 justify-between items-center">
          
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            
            {/* Promotion Dropdown */}
            <div className="relative">
              <button onClick={() => toggleDropdown('promotion')} className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${selectedPromotions.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
                แคมเปญ {selectedPromotions.length > 0 && `(${selectedPromotions.length})`}
                ...
                <div className="absolute top-full left-0 mt-3 w-64 bg-white/95 backdrop-blur-2xl border border-gray-100 rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {promotions.map((promo) => (
                      <li key={promo.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(promo.title, selectedPromotions, setSelectedPromotions)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedPromotions.includes(promo.title)} readOnly />
            ...
            {/* Category Dropdown */}
            <div className="relative">
              <button onClick={() => toggleDropdown('category')} className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${selectedCategories.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
              ...
              {activeDropdown === 'category' && (
                <div className="absolute top-full left-0 mt-3 w-64 bg-white/95 backdrop-blur-2xl border border-gray-100 rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {categories.map((cat) => (
                      <li key={cat.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(cat.name, selectedCategories, setSelectedCategories)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedCategories.includes(cat.name)} readOnly />
                        <span className={selectedCategories.includes(cat.name) ? 'font-bold text-red-500' : ''}>{cat.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {/* Room Dropdown */}
            <div className="relative">
              <button onClick={() => toggleDropdown('room')} className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${selectedRooms.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
              ...
              {activeDropdown === 'room' && (
                <div className="absolute top-full left-0 mt-3 w-64 bg-white/95 backdrop-blur-2xl border border-gray-100 rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {rooms.map((room) => (
                      <li key={room.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(room.name, selectedRooms, setSelectedRooms)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedRooms.includes(room.name)} readOnly />
                        <span className={selectedRooms.includes(room.name) ? 'font-bold text-red-500' : ''}>{room.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {/* Feature Dropdown */}
            <div className="relative">
              <button onClick={() => toggleDropdown('feature')} className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${selectedFeatures.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
              ...
              {activeDropdown === 'feature' && (
                <div className="absolute top-full left-0 mt-3 w-64 bg-white/95 backdrop-blur-2xl border border-gray-100 rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {features.map((feat) => (
                      <li key={feat.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(feat.name, selectedFeatures, setSelectedFeatures)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedFeatures.includes(feat.name)} readOnly />
                        <span className={selectedFeatures.includes(feat.name) ? 'font-bold text-red-500' : ''}>{feat.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {/* Price Filter */}
            <div className="flex items-center gap-2 bg-slate-50 rounded-full px-5 py-2 focus-within:ring-2 ring-red-400/50 transition-all border border-slate-200/50">
            ...
            <div className="relative">
              <button 
                ...
                ${!isAdvancedFilterAllowed ? 'opacity-50 cursor-not-allowed bg-gray-100 text-gray-400' : selectedColors.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
              >
              ...
              {activeDropdown === 'color' && isAdvancedFilterAllowed && (
                <div className="absolute top-full left-0 mt-3 w-48 bg-white/95 backdrop-blur-2xl border border-white rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {colors.map((color) => (
                      <li key={color.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(color.name, selectedColors, setSelectedColors)}>
                        <div className="w-4 h-4 rounded-full border border-gray-300 shadow-inner" style={{ backgroundColor: getColorHex(color.name) }}></div>
                        <span className={selectedColors.includes(color.name) ? 'font-bold text-red-500' : ''}>{color.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="relative">
              <button 
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all 
                ${!isAdvancedFilterAllowed ? 'opacity-50 cursor-not-allowed bg-gray-100 text-gray-400' : selectedMaterials.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
              >
              ...
              {activeDropdown === 'material' && isAdvancedFilterAllowed && (
                <div className="absolute top-full left-0 mt-3 w-48 bg-white/95 backdrop-blur-2xl border border-white rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {materials.map((mat) => (
                      <li key={mat.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(mat.name, selectedMaterials, setSelectedMaterials)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedMaterials.includes(mat.name)} readOnly />
                        <span className={selectedMaterials.includes(mat.name) ? 'font-bold text-red-500' : ''}>{mat.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="relative">
              <button 
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all 
                ${!isAdvancedFilterAllowed ? 'opacity-50 cursor-not-allowed bg-gray-100 text-gray-400' : selectedSizes.length > 0 ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
              >
              ...
              {activeDropdown === 'size' && isAdvancedFilterAllowed && (
                <div className="absolute top-full left-0 mt-3 w-48 bg-white/95 backdrop-blur-2xl border border-white rounded-2xl shadow-2xl p-5 z-50">
                  <ul className="max-h-60 overflow-y-auto space-y-4">
                    {sizes.map((size) => (
                      <li key={size.id} className="flex items-center gap-3 text-slate-600 hover:text-red-500 cursor-pointer" onClick={() => handleToggle(size.name, selectedSizes, setSelectedSizes)}>
                        <input type="checkbox" className="rounded-md border-slate-300 text-red-500 focus:ring-red-500 cursor-pointer" checked={selectedSizes.includes(size.name)} readOnly />
                        <span className={selectedSizes.includes(size.name) ? 'font-bold text-red-500' : ''}>{size.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {hasAnyFilter && (
              <button onClick={clearAllFilters} className="px-4 py-2.5 text-red-500 text-sm font-bold flex items-center gap-2 hover:bg-red-50 rounded-full transition-colors ml-auto lg:ml-0">
          ...
          <div className="flex flex-col items-end w-full lg:w-80 flex-shrink-0 mt-2 lg:mt-0">
            <div className="relative w-full group">
              <input 
                type="text" 
                placeholder="ค้นหาสินค้าราคาพิเศษ..." 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200/50 rounded-full focus:outline-none focus:ring-2 focus:ring-red-400 transition-all text-sm font-medium"
              />
              <Search className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-red-500 transition-colors" size={18} />
            </div>
            {hasAnyFilter && (
              <div className="text-xs text-slate-500 mt-2 pr-2 font-medium">
                ค้นพบ <span className="text-red-500 font-bold text-sm">{filteredProducts.length}</span> รายการ
              </div>
            )}
          </div>
        </div>
