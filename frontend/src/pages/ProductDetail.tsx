...
const ProductDetail = () => {
  ...
  const currentVariant = useMemo(() => {
    ...
    return product.variants.find(v => v.color === selectedColor && v.material === selectedMaterial && v.size === selectedSize);
  ...
  const images: string[] = useMemo(() => {
    ...
    if (rawMain) {
        if (Array.isArray(rawMain)) allImages = [...allImages, ...rawMain];
            ...
            } else allImages.push(rawMain);
        }
    }
    if (product.variants && Array.isArray(product.variants)) {
        product.variants.forEach((v: any) => { if (v.image && !allImages.includes(v.image)) allImages.push(v.image); });
    }
    return allImages.length === 0 ? ["https://via.placeholder.com/600x400?text=No+Image"] : allImages;
  }, [product]);
  ...
  const handleImageInteract = (index: number) => {
      ...
      if (matchedVariant) {
          ...
          setSelectedColor(mainColor); setSelectedMaterial(mainMaterial); setSelectedSize(mainSize);
      }
  };
  ...
  const isMainProductSelected = selectedColor === mainColor && selectedMaterial === mainMaterial && selectedSize === mainSize;
    ...
    <div className="bg-gray-50 min-h-screen pb-10 font-sans">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="text-sm text-gray-500 mb-4 flex items-center gap-2">
           ...
           <ChevronRight size={14}/> <span className="cursor-default">สินค้า</span> <ChevronRight size={14}/> 
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
          ...
          <ProductImageGallery 
            ...
            handleImageInteract={handleImageInteract} 
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
               ...
               <div className="mt-3 pt-3 border-t border-gray-200 flex flex-wrap gap-4 text-sm text-gray-700">
                   ...
                   <div><span className="font-bold text-gray-500">วัสดุ:</span> <span className={!displayMaterialAttr ? "text-gray-400" : ""}>{displayMaterialAttr || '-'}</span></div>
                   <div><span className="font-bold text-gray-500">ขนาด:</span> <span className={!displaySizeAttr ? "text-gray-400" : ""}>{displaySizeAttr || '-'}</span></div>
               </div>
            </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {availableColors.map(color => (
                                    ...
                                    >
                                        <div className="w-3 h-3 rounded-full border border-gray-300 shadow-inner" style={{ backgroundColor: getColorHex(color) }}></div>{color}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {availableMaterials.map(mat => (
                                    <button 
                                        key={mat} onClick={() => setSelectedMaterial(mat)}
                                    ...
                                    >
                                        {mat} {selectedMaterial === mat && <Check size={14} className="inline ml-1" />}
                                    </button>
                                ))}
                            </div>
                        </div>
               ...
               <div>
                  ...
                  <div className="flex items-center justify-between border border-gray-200 p-3 rounded-lg w-full xl:w-80">
                     ...
                     <div className="flex items-center bg-gray-50 border border-gray-300 rounded overflow-hidden">
                        <button onClick={() => setInstallationQty(Math.max(0, installationQty - 1))} className="p-1.5 hover:bg-white text-gray-500"><Minus size={14}/></button>
                        <span className="w-8 text-center text-sm font-bold py-1">{installationQty}</span>
                        <button onClick={() => setInstallationQty(Math.min(quantity, installationQty + 1))} className="p-1.5 hover:bg-white text-gray-500"><Plus size={14}/></button>
                     </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 pt-4">
                <div>
                    ...
                    <div className="flex items-center border border-gray-300 rounded overflow-hidden bg-white">
                      <button onClick={() => { const newQty = Math.max(1, quantity - 1); setQuantity(newQty); if (installationQty > newQty) setInstallationQty(newQty); }} className="p-3 hover:bg-gray-100"><Minus size={16}/></button>
                      ...
                      <button onClick={() => setQuantity(Math.min(Number(displayStock), quantity + 1))} className="p-3 hover:bg-gray-100" disabled={quantity >= Number(displayStock)}><Plus size={16}/></button>
                    </div>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-8">
             <div>
                ...
                <div className="prose max-w-none text-gray-600 whitespace-pre-line">{product.description || "ไม่มีรายละเอียดสินค้า"}</div>
             </div>

             {/* Reviews Component */}
             <ProductReviews reviews={reviews} API_BASE_URL={API_BASE_URL} />
        </div>
      </div>
    </div>
  );
